...
---""")
    ...
    with st.sidebar.header("Input data"):
        ...
        uploaded_files = st.sidebar.file_uploader(
            "Upload 3 input files for epitope", accept_multiple_files=True
        )
    ...
    if verified:
        ...
        with st.spinner("Loading input tables..."):
            peptide, mutation_data, AA_mutation_matrix, NA_mutation_matrix = LoadData(
                uploaded_files
            )
...
def KeyStatisticsSection(peptide, mutation_data):
    ...
    st.write(
        '<p style="font-size:22px;weight:bold">1. General sample statistics</p>',
    ...
    proteome = ReadProteinsFromFile(
        f"{script_dir}/reference_sequences/protein_sequences_reference.fasta"
    )
    ...
    # print peptide info
    st.write(
        f"Peptide {peptide.name}, sequence: **{peptide.sequence}** ({len(peptide)} residues)"
    )
    ...
    with st.container():
        ...
        with col1:
            ...
            key_stat = GetKeyStat(mutation_data, stat_only_metadata)

            # print statistics
            st.write(f"**Data for peptide {peptide.name} contains:**")
            st.write(f"{key_stat['Total']} samples")
            st.write(
                f"of them {key_stat['Has_metadata']} have metadata ({round(key_stat['Has_metadata']*100/key_stat['Total'], 1)}% of all)"
            )

            # in case we can provide clear date range
            some_samples_lack_metadata = (
                True if 0 in mutation_data["has_metadata"] else False
            )
            ...
            total = (
                key_stat["Has_metadata"] if stat_only_metadata else key_stat["Total"]
            )
            st.markdown(
...
def KeyStatisticsPlotSection(mutation_data, no_metadata):
    ...
    """
    st.write(
        '<p style="font-size:22px;weight:bold">2. General sample statistics in time</p>',
        unsafe_allow_html=True,
    )

    if no_metadata:
        st.write(
            '<p style="font-size:18px">No samples with metadata to plot</p>',
            unsafe_allow_html=True,
        )
    else:
        ...
        with st.spinner("Generating weekly stat plot..."):
            key_stat_plot = PlotKeyStat(mutation_data, plot_proportion=plot_proportion2)
...
def MutationStatSection(peptide, mutation_data, no_metadata):
    ...
    """
    st.write(
        '<p style="font-size:22px;weight:bold">3. Stat mutations</p>',
        unsafe_allow_html=True,
    )

    # whether to stat samples without metadata
    stat_only_metadata = st.checkbox(
        "Only stat samples with metadata",
    ...
    with st.container():
        ...
        with col1:
            stat_as = st.selectbox(
                "Stat mutations as", ("individual", "combinations"), key=2
            )
        ...
        with col4:
            end_date = st.date_input(
                "and up to:", def_end_date, key="d2", disabled=not stat_only_metadata
            )
            end_date = datetime(end_date.year, end_date.month, end_date.day)
        with col5:
            plot_as3 = st.selectbox(
                "Plot as:",
                ("counts", "proportion"),
                key=3,
                disabled=not stat_only_metadata,
    ...
    with st.container():
        ...
        with st.spinner("Generating mutation summary..."):
            # filter mutation data by metadata and dates
            if stat_only_metadata:
                filtered_mutation_data = mutation_data[
                    mutation_data["has_metadata"] == 1
                ]
                filtered_mutation_data = filtered_mutation_data[
                    filtered_mutation_data["sample_date"] >= start_date
                ]
                filtered_mutation_data = filtered_mutation_data[
                    filtered_mutation_data["sample_date"] <= end_date
                ]
            else:
                filtered_mutation_data = mutation_data
            ...
            with col1:
                st.write(
                    f"**Mutations summarised from {filtered_mutation_data.shape[0]} samples:**"
                )
                st.write(summary)
        ...
        if stat_only_metadata:
            ...
            with col2:
                ...
                top_found_mutations = (
                    found_mutations[:5] if len(found_mutations) > 5 else found_mutations
                )
                selected_mutations = st.multiselect(
                    "Mutations to plot:", found_mutations, top_found_mutations
                )
    # plot selected mutations in time
    if stat_only_metadata:
        with st.spinner("Generating timeline plot..."):
            # generate plot
            muts_time_plot = PlotMutationStat(
...
def SequenceConservationSection(NA_mutation_matrix, AA_mutation_matrix):
    ...
    """
    with st.spinner("Generating sequence conservation plot..."):
        sequence_conservation_plot = PlotSeqConservation(
            NA_mutation_matrix, AA_mutation_matrix
        )
...
def LineageStatSection(peptide, mutation_data, no_metadata):
    ...
    """
    st.write(
        '<p style="font-size:22px;weight:bold">4. Stat mutations by lineage</p>',
        unsafe_allow_html=True,
    )

    if no_metadata:
        st.write(
            '<p style="font-size:18px">No samples with metadata to stat</p>',
            unsafe_allow_html=True,
        )
    else:
        ...
        with st.container():
            ...
            with col1:
                start_date = st.date_input(
                    "Stat starting from:", def_start_date, key="d4"
                )
                start_date = datetime(start_date.year, start_date.month, start_date.day)
            with col2:
                ...
                end_date = datetime(end_date.year, end_date.month, end_date.day)
            with col3:
                report_as = st.selectbox(
                    "Report samples as:", ("counts", "proportion"), key=4
                )
                report_proportion = True if report_as == "proportion" else False

        # select mutations to stat
        with st.spinner("Generating lineage statistics..."):
            summary2 = MutationTextSummary(
                peptide,
                mutation_data[mutation_data["has_metadata"] == 1],
                stat_mutations=0,
                sort_by=0,
