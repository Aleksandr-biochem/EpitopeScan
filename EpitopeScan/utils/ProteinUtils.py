class Protein:
        ...
        for protein in proteins:
            ...
            for i in range(0, len(protein) - len(self) + 1):
                ...
                if sebsequence == self.sequence:
                    ...
                    genome_end_coordinate = (
                        genome_start_coordinate + (len(self) * 3) - 1
                    )
                    ...
                    # check the case of multiple match
                        ...
                        # in case of ambiguous location
                            ...
                            if verbose:
                                ...
                                print(
                                    f"First located at {self.genome_start}({self.parent_protein[-1]}), then at {protein.genome_start + (i * 3)}({protein.name})"
                                )
    ...
    def AssignCodingSequence(self, reference_genome):
        ...
        if (self.genome_start is not None) and (self.genome_start != -1):
            self.coding_sequence = reference_genome[
                self.genome_start - 1 : self.genome_end
            ]
        else:
            print(
                f"Warning! Cannot assign coding sequence to {self.name} due to indefinite coordinates"
            )
        ...
        for line in f:
            ...
            if line.startswith(">"):
                ...
                # create Protein instance from coordinate input
                    ...
                    if len(name_data) != 4:
                        raise Exception(
                            f"Expected 'name,parent_protein,start,end'. Got {','.join(name_data)}"
                        )
                    else:
                        ...
                        if seq != "":
                            ...
                            new_protein.LocateFromCoordinates(
                                parent_protein, start_res, proteome
                            )
                        else:
                            raise Exception(
                                f"Couldnot locate peptide {','.join(name_data)}. Check parent protein name and coordinates"
                            )
            ...
            # sequence line in FASTA file
                ...
                if len(set(seq) - set("GALMFWKQESPVICYHRNDT")) > 0:
                    print(
                        f"Warning! Protein {name} sequence contains unrecognised characters"
                    )
                else:
                    new_protein = Protein(name, seq)
...
def LoadPeptideInput(single_epitope, epitopes_file, proteome, ref_genome, verbose=True):
    ...
    if single_epitope is not None:
        ...
        # name + parent protein + coodinates
            ...
            if seq != "":
                epitopes_to_scan = [
                    Protein(name, seq).LocateFromCoordinates(
                        parent_protein, start_res, proteome
                    )
                ]
            else:
                raise Exception(
                    "Could not locate peptide. Check parent protein name and coordinates"
                )
    ...
    elif epitopes_file is not None:
