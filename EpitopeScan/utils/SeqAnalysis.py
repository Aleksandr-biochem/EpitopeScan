...
def CompareAAsequences(
    ...
    if (peptide.genome_start < 13469 - 1) and (13469 < peptide.genome_end):
        refrnc_seq = (
            refrnc_seq[: 13469 - peptide.genome_start]
    ...
    if partially_spanning_deletions_found:
        ...
        refrnc_protein, sample_protein = GlobalPtoteinAlignment(
            refrnc_protein, sample_protein
        )
...
def ReportSequenceMuatations(
    peptide, genome_seq, reference_genome, codon_table, ambiguity_threshold
):
    """
    ...
    sample_seq_for_NA_comparison = genome_seq[
        peptide.genome_start - 1 : peptide.genome_end
    ]
    ...
    if (peptide.parent_protein[0] == "Plp1ab") and (
        (peptide.genome_start < orf_shift_coord)
        and (orf_shift_coord < peptide.genome_end)
    ):
        sample_seq_for_AA_comparison = (
            genome_seq[peptide.genome_start - 1 : orf_shift_coord - 1]
            + genome_seq[orf_shift_coord - 2 : peptide.genome_end]
        )
    else:
        sample_seq_for_AA_comparison = genome_seq[
            peptide.genome_start - 1 : peptide.genome_end
        ]
    ...
    if partially_spanning_deletions_found:
        ...
        if sample_seq_for_AA_comparison[0] == "-":
            ...
            if del_end_j > 0:
                ...
                sample_seq_for_AA_comparison = (
                    genome_seq[pos - 3 : pos + (3 * (add_left - 1))]
                    + sample_seq_for_AA_comparison
                )
    ...
    AA_mutations = CompareAAsequences(
        peptide,
        sample_seq_for_AA_comparison,
        codon_table,
        ambiguity_threshold,
    ...
    if AA_mutations == ["-"]:
        ...
        NA_mutations = CompareNAsequences(peptide, sample_seq_for_NA_comparison)
...
def ExpandMutationData(dfs):
    ...
    for i in range(len(dfs)):
        ...
        for muts in dfs[i]["AA_mutations"]:
            if (muts is not None) and (muts not in ["-", "NF"]):
        ...
        AA_substitution_df = pd.DataFrame(
            0, index=np.arange(dfs[i].shape[0]), columns=list(unique_AA_mutations)
        )
...
def BindMetadata(dfs, metadata_file, sample_tag):
    ...
    if metadata_file is not None:
        ...
        metadata = metadata[
            ["sequence_name", "sample_date", "epi_week", "usher_lineage"]
        ]
        ...
        for i in range(len(dfs)):
            ...
            dfs[i]["sample_date"] = pd.to_datetime(
                dfs[i]["sample_date"], format="%Y-%m-%d"
            )

        print("Metadata binding finished")
        print(f"Metadata found for {sum(dfs[0]['has_metadata'] == 1)}")
        no_metadata = sum(dfs[0]["has_metadata"] == 0)
        print(
            f"{no_metadata} samples ({round(no_metadata*100/dfs[0].shape[0], 1)}%) lack metadata\n"
        )
...
def CheckFrameDisruption(orf_start, orf_end, genome_sequence):
    ...
    if (len(set(ORF_sequence[:3]) - set("ATCG-")) == 0) and ORF_sequence[:3] != "ATG":
        return True
    if (len(set(ORF_sequence[-3:]) - set("ATCG-")) == 0) and (
        ORF_sequence[-3:] not in ["TAA", "TGA", "TAG"]
    ):
        return True
...
def ScanMSA(
    ...
    for epitope in epitopes_to_scan:
        ...
        epitope.NA_mutations_matrix = np.zeros(
            (len(epitope.coding_sequence), 5), dtype=int
        )

    # read Open Reading Frames coordinates
    # for ORF disruption control
    ORF_coordinates = dict()
    file_orf_coordinates = pkg_resources.resource_filename(
        "EpitopeScan", "reference_sequences/ORFs_reference.txt"
    )
    ...
    filter_samples = True if (sample_tag is not None) else False
            ...
            # genome sequence line
                ...
                if not skip_sample:
                    ...
                    if quality_filter is not None:
                        proportion_of_N = genome_aln.count("N") / (
                            len(genome_aln) - genome_aln.count("-")
                        )
                    ...
                    for epitope in epitopes_to_scan:
                        if epitope.parent_protein[0] not in frame_disruption.keys():
                            disruption = CheckFrameDisruption(
                                ORF_coordinates[epitope.parent_protein[0]][0],
    ...
    if verbose:
        ...
        if quality_filter is not None:
            print(
                f"Rejected {discarded_by_quality} genomes having >{quality_filter} of N bases"
            )
    ...
    for i in range(1, len(df[0])):
        ...
        new_df = pd.DataFrame(
            epitope_data, columns=["sequence_name", "NA_mutations", "AA_mutations"]
        )
        dfs.append(new_df)
