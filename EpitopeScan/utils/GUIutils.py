...
def VerifyInput(files):
    ...
    if len(files) > 3:
        ...
        if len(names) > 1:
            st.sidebar.write(
                "Input files seem to belong to different epitopes. Revise input"
            )
        else:
            name = names.pop()
            if f"{name}_AA_mutation_matrix.csv" not in file_names:
                st.sidebar.write(f"Missing {name}_AA_mutation_matrix.csv")
            elif f"{name}_NA_mutation_matrix.csv" not in file_names:
                st.sidebar.write(f"Missing {name}_NA_mutation_matrix.csv")
            elif f"{name}_mutation_data.tsv" not in file_names:
...
def LoadData(files):
    ...
    for i in range(len(files)):
        if files[i].name == f"{peptide_name}_AA_mutation_matrix.csv":
            ...
            mutation_data["sample_date"] = pd.to_datetime(
                mutation_data["sample_date"], format="%Y-%m-%d"
            )
...
def KeyPieChart(key_stat):
    ...
    fig.add_trace(go.Pie(labels=list(key_stat.keys()), values=list(key_stat.values())))

    fig.update_traces(textfont_size=20)
    fig.update_layout(
        title_text="Samples breakdown",
        title_x=0.5,
        title_font_size=20,
        legend_font_size=15,
        height=500,
...
def KeyWeeklyStat(df, report_proportion=False):
    ...
    # total samples
    grouped_to = (
        df.groupby("epi_week", as_index=False)
        .count()[["epi_week", "sequence_name"]]
    ...
    stat = (
        week_column.merge(grouped_nf, on="epi_week", how="left")
...
def PlotKeyStat(mutation_data, plot_proportion=False):
    ...
    for value in stat.columns[1:]:
        fig.add_trace(go.Bar(x=stat["epi_week"], y=stat[value], name=value))

    fig.update_layout(
        title_text="Samples statistics per week",
        title_x=0.5,
...
@st.cache_data
def StatMutations(
    df, mutations, report_proportion=False, time_start=None, time_end=None
):
    """
...
@st.cache_data
def PlotMutationStat(
    df, mutations, plot_proportion=False, time_start=None, time_end=None
):
    """
    ...
    for col in stat.columns[i:]:
        fig.add_trace(go.Bar(x=stat["epi_week"], y=stat[col], name=col))

    fig.update_layout(
        title_text="Mutations statistics per week",
        title_x=0.5,
        xaxis_title="Pandemic week",
    )
    ...
    if plot_proportion:
        ...
        fig.add_trace(
            go.Bar(x=stat["epi_week"], y=stat["n_samples"], name="Total samples")
        )
        fig.update_layout(yaxis_title="Count")
...
def PlotSeqConservation(NA_matrix, AA_matrix):
    ...
    AA_matrix["Conservation"] = AA_matrix.apply(
        lambda row: row[row.name] / row.sum(), axis=1
    )
    ...
    fig.append_trace(
        go.Bar(
            x=[i for i in range(len(AA_matrix.index))],
    ...
    for b in "ATCGΔ":
        fig.append_trace(
            go.Bar(x=[i for i in range(len(NA_matrix.index))], y=NA_matrix[b], name=b),
            row=2,
            col=1,
        )
    ...
    fig.update_layout(
        title_text="Sequence conservation",
        title_x=0.5,
        showlegend=False,
        barmode="stack",
        height=500,
...
@st.cache_data
def StatLineages(df, mutations, report_proportion, time_start=None, time_end=None):
    """
    ...
    lineage_counts = (
        df.groupby("usher_lineage", as_index=False)
    ...
    for mut in mutations:
        ...
        mutation_counts = (
            df[mask]
            .groupby("usher_lineage", as_index=False)
            .count()[["usher_lineage", "sequence_name"]]
            .rename(columns={"sequence_name": mut})
        )
