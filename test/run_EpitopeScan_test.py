        ...
        if row["NA_mutations"] == "NF":
            ...
            if ref_data[name]["non_functional"]:
                ...
                if NA_mutations != set(ref_data[name]["NA_mutations"]):
                    print(
                        f"{name} NA mutation mismatch:",
                        NA_mutations,
                        " ",
                        ref_data[name]["NA_mutations"],
                    )
                    mismatched_samples.append(name)
                if AA_mutations != set(ref_data[name]["AA_mutations"]):
                    print(
                        f"{name} AA mutation mismatch:",
                        AA_mutations,
                        " ",
                        ref_data[name]["AA_mutations"],
                    )
                    mismatched_samples.append(name)
...
if __name__ == "__main__":
    ...
    for peptide_name in peptide_names:
        seq = "".join(
            pd.read_csv(
                f"{script_dir}/{peptide_name}/{peptide_name}_AA_mutation_matrix.csv",
                index_col=0,
            ).index
        )
    ...
    for name, peptide in zip(peptide_names, test_peptides):
        ...
        ref_NA_matrix = pd.read_csv(
            f"{script_dir}/{name}/{name}_NA_mutation_matrix.csv", index_col=0
        )
        ref_AA_matrix = pd.read_csv(
            f"{script_dir}/{name}/{name}_AA_mutation_matrix.csv", index_col=0
        )
        ...
        subprocess.run(
            [
                ...
                f"{name},{peptide}",
                "--msa",
                f"{script_dir}/{name}/{name}_test_sequences.fasta",
                "-o",
                f"ES_{name}",
            ],
        ...
        if (
            (len(mistmatches) == 0)
            and NA_count_comparison.empty
            and AA_count_comparison.empty
        ):
            print("Test passed\n")
        else:
            print("Test failed!\n")
        ...
        # compare mutation matrices
        mistmatches = CompareMutationData(
            mut_data, ref_data, ambiguity_intolerance=True
        )
